...
KRUN_MAC="02:53:50:52:4b:08"
PLUGIN_INTERFACE="spr-masque"
curl --fail-with-body --silent --show-error "http://127.0.0.1/device?identity=${KRUN_MAC}" \
  ...
  -X PUT --data-raw "{\"MAC\":\"${KRUN_MAC}\",\"Name\":\"spr-masque\",\"Policies\":[\"wan\",\"dns\"],\"Groups\":[\"masque\"]}" >/dev/null
...
[ -n "$CONTAINER_IP" ] || { echo "spr-masque did not obtain an SPR DHCP lease" >&2; exit 1; }
API=127.0.0.1
...
-X 'PUT' \
--data-raw "{\"SrcIP\":\"${CONTAINER_IP}\",\"Interface\":\"${PLUGIN_INTERFACE}\",\"Policies\":[\"wan\",\"dns\"],\"Groups\":[\"masque\"]}"
