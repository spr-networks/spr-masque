...
WORKDIR /code
COPY code/ /code/
RUN --mount=type=tmpfs,target=/root/go/ go build -trimpath -ldflags "-s -w" -o /masque_plugin /code/

# usque requires go >= 1.26.3 (go.mod), so it gets its own pinned toolchain.
FROM ${UBUNTU_REF} AS builder-usque
ENV DEBIAN_FRONTEND=noninteractive
ARG UBUNTU_SNAPSHOT=20260601T000000Z
ARG USQUE_GO_VERSION=1.26.5
ARG USQUE_GO_SHA256_AMD64=5c2c3b16caefa1d968a94c1daca04a7ca301a496d9b086e17ad77bb81393f053
ARG USQUE_GO_SHA256_ARM64=fe4789e92b1f33358680864bbe8704289e7bb5fc207d80623c308935bd696d49
ARG USQUE_COMMIT=0fa6da9e34714a13676361afbfe6a58ad6597110
ARG TARGETARCH
COPY --from=cacerts /etc/ssl/certs/ca-certificates.crt /etc/ssl/certs/ca-certificates.crt
RUN set -eux; \
    printf 'Types: deb\nURIs: https://snapshot.ubuntu.com/ubuntu/%s\nSuites: noble noble-updates noble-security\nComponents: main restricted universe multiverse\nSigned-By: /usr/share/keyrings/ubuntu-archive-keyring.gpg\n' "${UBUNTU_SNAPSHOT}" > /etc/apt/sources.list.d/ubuntu.sources; \
...
RUN set -eux; \
    case "${TARGETARCH}" in \
      amd64) GO_SHA256="${USQUE_GO_SHA256_AMD64}";; \
      arm64) GO_SHA256="${USQUE_GO_SHA256_ARM64}";; \
      *) echo "unsupported TARGETARCH=${TARGETARCH}" >&2; exit 1;; \
    esac; \
    wget -q "https://dl.google.com/go/go${USQUE_GO_VERSION}.linux-${TARGETARCH}.tar.gz"; \
    echo "${GO_SHA256}  go${USQUE_GO_VERSION}.linux-${TARGETARCH}.tar.gz" | sha256sum -c -; \
    tar -C /usr/local -xzf "go${USQUE_GO_VERSION}.linux-${TARGETARCH}.tar.gz"; \
    rm "go${USQUE_GO_VERSION}.linux-${TARGETARCH}.tar.gz"
ENV PATH="/usr/local/go/bin:${PATH}" GOTOOLCHAIN=local
WORKDIR /usque
RUN set -eux; \
    git init .; \
    git remote add origin https://github.com/Diniboy1123/usque.git; \
    git fetch --depth 1 origin "${USQUE_COMMIT}"; \
    git checkout FETCH_HEAD; \
    [ "$(git rev-parse HEAD)" = "${USQUE_COMMIT}" ]
RUN --mount=type=tmpfs,target=/root/go/ CGO_ENABLED=0 go build -trimpath -ldflags "-s -w" -o /usque_bin .
...
COPY --from=builder /masque_plugin /
COPY --from=builder-usque /usque_bin /usr/bin/usque
